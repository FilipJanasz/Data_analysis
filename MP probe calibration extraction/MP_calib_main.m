function [file,data]=MP_calib_main(clear_flag,filePath_default)
%%  PICK A FOLDER FOR PROCCESSING  
         
    %display gui to pick directory
    oldCd=cd;
    cd('D:\Movable Probe\Probe Calibration_Guillaume')
    directories = uigetdir('D:\Movable Probe\Probe Calibration_Guillaume','Pick a directory');
    cd(oldCD);
    try
...
%             [file(fc),data(fc)]=MP_processing_fun(file_list{dir_counter}{fc},directories{dir_counter});
            [fileTemp,dataTemp]=MP_processing_fun(file_list{dir_counter}{fc},directories{dir_counter});

            file(fc)=fileTemp;
            data(fc)=dataTemp;
            % below is for debugging
%            [steam1, coolant1, facility1, NC1, distributions1, file1,BC1, MP1, MP1]=file_processing(interactive_flag,file_list{dir_counter}{fc},directories{dir_counter},st_state_flag,options);
%            steam(fc)=steam1;
%            coolant(fc)=coolant1;
