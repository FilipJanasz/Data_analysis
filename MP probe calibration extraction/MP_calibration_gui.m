function varargout = MP_calibration_gui(varargin)
    % profile on
    clc
...
%     cd ..s
    ...
    % MP_CALIBRATION_GUI MATLAB code for MP_calibration_gui.fig
    %      MP_CALIBRATION_GUI, by itself, creates a new MP_CALIBRATION_GUI or raises the existing
    %      singleton*.
    %
    %      H = MP_CALIBRATION_GUI returns the handle to a new MP_CALIBRATION_GUI or the handle to
    %      the existing singleton*.
    %
    %      MP_CALIBRATION_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in MP_CALIBRATION_GUI.M with the given input arguments.
    %
    %      MP_CALIBRATION_GUI('Property','Value',...) creates a new MP_CALIBRATION_GUI or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the MP_CALIBRATION_GUI before MP_calibration_gui_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to MP_calibration_gui_OpeningFcn via varargin.
    %
    %      *See MP_CALIBRATION_GUI Options on GUIDE's Tools menu.  Choose "MP_CALIBRATION_GUI allows only one
    %      instance to run (singleton)".
    %

    % Edit the above text to modify the response to help MP_calibration_gui
    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @MP_calibration_gui_OpeningFcn, ...
                       'gui_OutputFcn',  @MP_calibration_gui_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before MP_calibration_gui is made visible.
function MP_calibration_gui_OpeningFcn(hObject, ~, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to MP_calibration_gui (see VARARGIN)
    % Choose default command line output for MP_calibration_gui
...
% profile viewer
    % UIWAIT makes MP_calibration_gui wait for user response (see UIRESUME)
    % uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = MP_calibration_gui_OutputFcn(~, ~, handles) 
...
%     filePath_default='D:\Movable Probe\Probe Calibration_Guillaume';
    % call function down the line for file processing
    [file,data]=MP_calib_main(clear_flag,filePath_default);
    ...
    assignin('base','MP_data',handles.data)
    assignin('base','MP_file',handles.file)
    
    % based on what variables are present, set possible choices to
    % popupmenus for plotting
%     vars={'steam','coolant','facility','NC','BC','MP','MP'};
    vars=fields(data);
    vars=vars';
...
%     profile on
    ...
    clear steam coolant facility NC distributions file BC MP MP timing
...
function clear_Callback(hObject, eventdata, handles)
   ...
   % clear screen
    ...
    %reset MP_CALIBRATION_GUI elements
...
function toolbar_save_fig_ClickedCallback(hObject, eventdata, handles)
    ...
    % 0. move to file directory, based on default value stored in MP_CALIBRATION_GUI
    filePath_default=get(handles.file_path_disp,'String');
    cd(filePath_default)
