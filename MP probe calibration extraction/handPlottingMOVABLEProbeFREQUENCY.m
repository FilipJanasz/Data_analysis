...
%     
    ...
    actualFreq(n)=6*(RPM/60);
%     title(num2str(actualFreq));
    
    A=ylim;
    hold on
%     plot([actualFreq actualFreq],[A(1),A(2)],'k--','LineWidth',1)
    xlabel('Frequency [Hz]')
    legendStr{n}=[num2str(actualFreq(n)),' [Hz]'];
...
% % 
% for m=1:n
%     plot([actualFreq(m) actualFreq(m)],[A(1),A(2)],'k--','LineWidth',1)
% %     xlabel('Frequency [Hz]')
% %     legendStr{n}=[num2str(actualFreq),' [Hz]'];
% end
...
print('D:\Data_analysis\figureOutputs\MPTempFREQ','-dmeta')
