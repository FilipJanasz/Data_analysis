function [v,cv,drho_dT,drho_dP]=f_h2o_properties(T,p)
...
PI=0.1*p/p_star; % 0.1 because of MPa to bar
tau=t_star/T;
gamma_pi=0;
gamma_pi_pi=0;
gamma_pi_tau=0;
...
alfaV=1/T*(1-tau*gamma_pi_tau/gamma_pi);
cv=R*(-tau^2*gamma_tau_tau+(gamma_pi-tau*gamma_pi_tau)^2/gamma_pi_pi);

%% GENERAL EQUATION FOR DERIVATIVES (GIBBS FREE ENERGY EQUATION)
% dzdx_y=(dzdp*dydt-dzdt*dydp)/(dxdp*dydt-dxdt*dydp);
...
% dpdp=1; 
% dpdt=0; 
% dvdt=(dvdp*dtdt-dvdt*dtdp)/(dpdp*dtdt-dpdt*dtdp);
...
% dtdp=0; 
% dtdt=1; 
% dvdT=(dvdp*dpdt-dvdt*dpdp)/(dtdp*dpdt-dtdt*dpdp);
