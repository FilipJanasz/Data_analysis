function writeCFD(steamFlow,steamTemp,clntFlow,clntTemp,clntPress,clntVel,initPress,initN2,initHe,fileName)
    ...
    fid=fopen('./fluent/2DMacroTemplate','r');
    i = 1;
    tline = fgetl(fid);
    A{i} = tline;
    while ischar(tline)
    ...
    for n=1:numel(steamFlow)
        ...
        temp{16}=['/solve/patch steam () species-0 ',inN2];
        temp{17}=['/define/operating-conditions/operating-pressure ',inPress];
        ...
        temp{18}=['/solve/patch coolant () pressure ',ctPress];
        temp{19}=['/solve/patch coolant () x-velocity n ',ctVel];   
        ...
        temp{20}=['/file/write-case \"/home/janasz_f/',fileName{n},'/',fileName{n},'.cas\" ok'];
        temp{21}=['/file/auto-save/root-name \"/home/janasz_f/',fileName{n},'/',fileName{n},'.cas\"'];
               
        %define plotting of data
        temp{25}=['/plot/plot/plot n \"/home/janasz_f/',fileName{n},'/h2o_centerline.txt\" yes n n h2o y 1 0 symmetry ()']; 
        temp{26}=['/plot/plot/plot n \"/home/janasz_f/',fileName{n},'/temp_centerline.txt\" yes n n temperature y 1 0 symmetry ()'];
        temp{27}=['/plot/plot n \"/home/janasz_f/',fileName{n},'/temp_wall.txt\" yes n n temperature y 1 0 steam_tube ()']; 
        temp{28}=['/plot/plot n \"/home/janasz_f/',fileName{n},'/h2o_wall.txt\" y n n h2o y 1 0 steam_tube ()']; 
        temp{29}=['/plot/plot n \"/home/janasz_f/',fileName{n},'/udm4_wall.txt\" y n n udm-4 y 1 0 steam_tube ()'];
